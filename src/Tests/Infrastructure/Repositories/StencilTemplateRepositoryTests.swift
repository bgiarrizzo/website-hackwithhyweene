...
struct StencilTemplateRepositoryTests {
    ...
    private func makeRepository(
        ...
        let menuFileURL = dir.appendingPathComponent("nav-menu.yml")
        try "menu:\n  - name: /blog\n    url: /blog/\n  - name: /liens\n    url: /liens/\n"
            .write(to: menuFileURL, atomically: true, encoding: .utf8)

        let repo = try StencilTemplateRepository(
            templatePath: dir.path,
            navigationMenuPath: menuFileURL.path
        )
        return (repo, dir)
    }
    ...
    @Test("render exposes YAML navigation menu to templates")
    func rendersNavigationMenuFromYAML() throws {
        let (repo, dir) = try makeRepository(
            templates: ["menu.html": "{% for item in menu %}{{ item.name }}{% endfor %}"])
        defer { try? FileManager.default.removeItem(at: dir) }

        let result = try repo.render(template: "menu.html", context: [:])

        #expect(result.contains("/blog"))
        #expect(result.contains("/liens"))
    }
